from app.cli_memory.controllers.create_profession_controller import (
    CreateProfessionController,
)
from interactor.dtos.create_profession_dtos import CreateProfessionInputDto
...
def test_create_profession(monkeypatch, mocker, fixture_profession_developer):
    ...
    fake_user_input = iter([name, description])
    monkeypatch.setattr("builtins.input", lambda _: next(fake_user_input))
    mock_repository = mocker.pathch(
        "src.app.cli_memory.controllers.create_profession_controller.ProfessionInMemoryRepository"
    )
    mock_presenter = mocker.patch(
        "src.app.cli_memory.controllers.create_profession_controller.CreateProfessionPresenter"
    )
    mock_use_case = mocker.patch(
        "src.app.cli_memory.controllers.create_profession_controller.CreateProfessionUseCase"
    )
    mock_use_case_instance = mock_use_case.return_value
    mock_view = mocker.patch(
        "src.app.cli_memory.controllers.create_profession_controller.CreateProfessionView"
    )
    result_use_case = {
